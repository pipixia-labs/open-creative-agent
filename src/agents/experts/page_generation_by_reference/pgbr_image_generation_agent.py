...
async def nano_banana_image_generation_tool(
    ...
    try:
        ...
        llm = LlmAgent(
            name="pgbr_image_generation_nano_banana",
            model="gemini-3.1-flash-image-preview",
            instruction="Generate image according to prompt and reference image.",
        ...
        if img_message is not None and len(img_message) > 0:
            result = {
                'status': "success",
                "image_bytes": img_message,
                "provider": "gemini",
                "model_name": "gemini-3.1-flash-image-preview",
                "usage": usage,
            }
            logger.info(f"nano_banana image generation completed. bytes={len(img_message)}")
        else:
            result = {
                'status': "error",
...
class PGBRImageGenerationAgent(BaseAgent):
    """Generate image assets for page recreation from reference material and prompts."""
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        draft_results = ctx.session.state.get('page_generation_by_reference/draft_results', {})
        logger.info(draft_results)
        draft_results = clean_json_string(draft_results)
        logger.info(draft_results)
        draft_results = json.loads(draft_results)
        # logger.info(draft_results)
        if 'reference_image_name' not in current_parameters or 'pgbr_image_to_generate' not in draft_results:
            error_text = f"提供给{self.name}的参数缺失，必须包含： reference_image_name 和 pgbr_image_to_generate"
        ...
        pgbr_image_to_generate = draft_results.get('pgbr_image_to_generate', [])
        logger.info(f'{len(pgbr_image_to_generate)} images to generate')

        result_image_list = []
        output_artifacts = []
        message = []
        pre_prompt = '''
你的任务是参考一个指定的图像，来根据提示词生成一个新的图像。
给定的参考图像通常是一个截图或者手机拍摄的图像，这个图像里面通常有若干图像元素，你的任务是生成其中的一个。**并不是生成给定的图像的全局**。
给你的提示词中有需要参考给定图像中哪个部分，以及期望的内容。

注意：
  - 你不需要生成参考图像的的全局
  - 你只需要参考那个局部图像，来根据提示词生成需要的图像。
  - 如果图像要求是透明的，一定要确保透明区域是真的透明，不要带上设计软件里的棋盘格背景。
  - 一定不要参考给定图像的全局！
  - 一定不要参考给定图像的全局!!
  - 一定不要参考给定图像的全局!!!
...
'''
        for i, img_info in enumerate(pgbr_image_to_generate):
            aspect_ratio = "1:1"
            resolution = "1K"
            prompt = ''
            # description = ''
            ...
            if 'position_on_screenshot' in img_info:
                position_on_screenshot = img_info['position_on_screenshot']
                if len(position_on_screenshot) > 0:
                    prompt = pre_prompt + '需要生成的图像对应到参考图像的位置：' + position_on_screenshot + '\n' + '参考图像其他区域不用关注（务必注意）。\n'
            ...
            if results['status'] == 'error':
                ...
                if file_name_placeholder:
                    ...
                    artifact_name = f"step{ctx.session.state.get('step') + 1}_pgbr_image_generation_output{i}_{file_name_placeholder_remove_dot}.png"
                else:
                    artifact_name = f"step{ctx.session.state.get('step') + 1}_pgbr_image_generation_output{i}.png"
        ...
        yield self.format_event(message, {"pgbr_image_generation_results": current_output})
        return
